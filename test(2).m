...
num_ec=18000;   %电解槽数量
num_b=90636.9645722471;  %蓄电池数量
num_c=7918476.00238331;
P_ecd=zeros(1,8640);
Load=zeros(1,8640);
xu=zeros(1,8640);
ch=zeros(1,8640);
    ...
    if i==1
      ...
      if P_exc(i)>=0 %负荷功率缺额
                    ...
                    if P_exc(i)<=P_dis_max  %case1:净负荷小于蓄电池最大功率且soc允许，由蓄电池补充缺额
                        ...
                        Eb(i)=E_bo;
                        if Ec(i)<E_cmin
                           Q_bre(i)=(E_cmin-Ec(i))*0.98+(P_exc(i)-P_disc(i));
                           Ec(i)=E_cmin;
                        else
                           Q_bre(i)=(P_exc(i)-P_disc(i)); 
                        end
                        Qc(i)=(E_co-Ec(i))*0.98;
                        Q_was(i)=0;
                   ...
                   elseif P_exc(i)>P_dis_max && P_exc(i)<=(P_dis_max+P_dis_maxc) %case2：净负荷大于蓄电池最大功率，小于蓄电池和主网售电最大功率了之和，蓄电池以最大功率放电，剩余不足由主网补充
                        ...
                        if tb(i)>=1&&tc(i)>=1
                            ...
                            Qb(i)=P_dis(i);
                            Qc(i)=P_disc(i);
                        elseif tb(i)<=tc(i)&&tb(i)<1%蓄电池先放电至最小容量
                            Eb(i)=E_bmin;
                            Qb(i)=(E_bo-E_bmin)*c_dis;
                            if P_exc(i)>=P_dis_maxc
                                Ec(i)=E_co-((P_exc(i)-P_dis(i))*tb(i)+P_dis_maxc*(1-tb(i)))/0.98;%不考虑容量限制，放电一个小时后的容量
                               if (E_co-P_disc(i)*tb(i)/0.98-E_cmin)*0.98/P_dis_maxc>(1-tb(i))%超级电容容量未达下限
                                   Q_bre(i)=(P_exc(i)-P_dis_maxc)*(1-tb(i));
                                   Qc(i)=P_disc(i)*tb(i)+P_dis_maxc*(1-tb(i));
                                   
                               else       %超级电容容量达下限
                                   Q_bre(i)=(P_exc(i)-P_dis_maxc)*(1-tb(i))+(E_cmin-Ec(i))*0.98;
                                   Qc(i)=(E_co-E_cmin)*c_disc;
                                   Ec(i)=E_cmin;
                               end
                            else  %功率缺额小于超级电容最大功率
                               Ec(i)=E_co-((P_exc(i)-P_dis(i))*tb(i)+P_exc(i)*(1-tb(i)))/0.98;
                                if (E_co-P_disc(i)*tb(i)/0.98-E_cmin)*0.98/P_exc(i)>(1-tb(i))%超级电容容量未达下限
                                   Q_bre(i)=0;
                                   Qc(i)=P_disc(i)*tb(i)+P_exc(i)*(1-tb(i));
                        ...
                        elseif tc(i)<tb(i)&&tc(i)<1  %%超级电容先放电至最小容量
                            Ec(i)=E_cmin;
                            Qc(i)=(E_co-E_cmin)*c_disc;
                            if tb(i)>1%%蓄电池未放电至最小容量
                                Q_bre(i)=(P_exc(i)-P_dis_max)*(1-tc(i));
                                Qb(i)=P_dis_max;
                            else%%蓄电池放电至最小容量
                                Q_bre(i)=(P_exc(i)-P_dis_max)*(1-tc(i))+(E_bmin-Eb(i))*c_dis;
                                Qb(i)=(E_bo-E_bmin)*c_dis;
                                Eb(i)=E_bmin;
                            end
                ...
                Q_bre(i)=0;
                    ...
                    if abs(P_exc(i))<=P_cha_max  %case1:净负荷小于蓄电池最大功率且soc允许，由蓄电池补充缺额
                        ...
                        if  P_cha_maxc>abs(P_exc(i))
                        P_chac(i)=abs(P_exc(i));
                        ...
                        Eb(i)=E_bo;
                        if Ec(i)>E_cmax
                           Q_was(i)=(Ec(i)-E_cmax)/0.98+(abs(P_exc(i))-P_chac(i));
                           Ec(i)=E_cmax;
                        else
                           Q_was(i)=(abs(P_exc(i))-P_chac(i)); 
                        end
                        Qc(i)=-(Ec(i)-E_co)/0.98;
                   ...
                   elseif abs(P_exc(i))>P_cha_max && abs(P_exc(i))<=(P_cha_max+P_cha_maxc) %case2：净负荷大于蓄电池最大功率，小于蓄电池和主网售电最大功率了之和，蓄电池以最大功率放电，剩余不足由主网补充
                        ...
                        if tb(i)>=1&&tc(i)>=1
                            ...
                            Qb(i)=-P_cha(i);
                            Qc(i)=-P_chac(i);
                        elseif tb(i)<=tc(i)&&tb(i)<1%蓄电池先充电至最大容量
                            Eb(i)=E_bmax;
                            Qb(i)=(E_bo-E_bmax)/c_cha;
                            if abs(P_exc(i))>=P_cha_maxc
                                Ec(i)=E_co+((abs(P_exc(i))-P_cha(i))*tb(i)+P_cha_maxc*(1-tb(i)))*0.98;%不考虑容量限制，放电一个小时后的容量
                               if (E_cmax-(E_co+P_chac(i)*tb(i)*0.98))/(P_cha_maxc*0.98)>(1-tb(i))%超级电容容量未达下限
                                   Q_was(i)=(abs(P_exc(i))-P_cha_maxc)*(1-tb(i));
                                   Qc(i)=-(P_chac(i)*tb(i)+P_cha_maxc*(1-tb(i)));
                                   
                               else       %超级电容容量达下限
                                   Q_was(i)=(abs(P_exc(i))-P_cha_maxc)*(1-tb(i))+(Ec(i)-E_cmax)/0.98;
                                   Qc(i)=(E_co-E_cmax)/c_chac;
                                   Ec(i)=E_cmax;
                               end
                            else  %功率缺额小于超级电容最大功率
                               Ec(i)=E_co+((abs(P_exc(i))-P_cha(i))*tb(i)+abs(P_exc(i))*(1-tb(i)))*0.98;
                                if (E_cmax-(E_co+P_chac(i)*tb(i)*0.98))/(abs(P_exc(i))*0.98)>(1-tb(i))%超级电容容量未达下限
                                   Q_was(i)=0;
                                   Qc(i)=-(P_chac(i)*tb(i)+abs(P_exc(i))*(1-tb(i)));
                        ...
                        elseif tc(i)<tb(i)&&tc(i)<1  %%超级电容先放电至最小容量
                            ...
                            Qc(i)=(E_co-E_cmax)/c_chac;
                            if tb(i)>1%%蓄电池未放电至最小容量
                                Q_was(i)=(abs(P_exc(i))-P_cha_max)*(1-tc(i));
                                Qb(i)=-P_cha_max;
                            else%%蓄电池放电至最小容量
                                Q_was(i)=(abs(P_exc(i))-P_cha_max)*(1-tc(i))+(Eb(i)-E_bmax)/c_cha;
                                Qb(i)=(E_bo-E_bmax)/c_cha;
                                Eb(i)=E_bmax;
                            end
                  ...
                  if P_exc(i)<=P_dis_max  %case1:净负荷小于蓄电池最大功率且soc允许，由蓄电池补充缺额
                   ...
                   elseif P_exc(i)>P_dis_max && P_exc(i)<=(P_dis_max+P_dis_maxc) %case2：净负荷大于蓄电池最大功率，小于蓄电池和主网售电最大功率了之和，蓄电池以最大功率放电，剩余不足由主网补充
                        ...
                        elseif tb(i)<=tc(i)&&tb(i)<1%蓄电池先放电至最小容量
                            ...
                            if P_exc(i)>=P_dis_maxc
                                Ec(i)=Ec(i-1)-((P_exc(i)-P_dis(i))*tb(i)+P_dis_maxc*(1-tb(i)))/0.98;%不考虑容量限制，放电一个小时后的容量
                               if (Ec(i-1)-P_disc(i)*tb(i)/0.98-E_cmin)*0.98/P_dis_maxc>(1-tb(i))%超级电容容量未达下限
                                   Q_bre(i)=(P_exc(i)-P_dis_maxc)*(1-tb(i));
                                   Qc(i)=P_disc(i)*tb(i)+P_dis_maxc*(1-tb(i));
                               ...
                               Ec(i)=Ec(i-1)-((P_exc(i)-P_dis(i))*tb(i)+P_exc(i)*(1-tb(i)))/0.98;
                                if (Ec(i-1)-P_disc(i)*tb(i)/0.98-E_cmin)*0.98/P_exc(i)>(1-tb(i))%超级电容容量未达下限
                                   Q_bre(i)=0;
                                   Qc(i)=P_disc(i)*tb(i)+P_exc(i)*(1-tb(i));
                    ...
                    elseif P_exc(i)>(P_dis_max+P_dis_maxc)  
                         ...
                         if Eb(i)<=E_bmin%%蓄电池达到容量下限
                            ...
                            if Eb(i-1)>E_bmin&&Ec(i-1)<=E_cmin%%只有蓄电池能放电
                               Ec(i)=E_cmin;
                               Qc(i)=0;
                               if tb(i)<=1%蓄电池容量达到上限
                                  Qb(i)=(Eb(i-1)-E_bmin)*c_dis;
                                  Q_bre(i)=(P_exc(i)-P_dis_max)+(E_bmin-(Eb(i-1)-P_dis(i)/c_dis))*c_dis;
                                  Eb(i)=E_bmin;
                               else
                                  
                                  Q_bre(i)=(P_exc(i)-P_dis_max);
                               end
                            elseif Eb(i-1)<=E_bmin&&Ec(i-1)>E_cmin%%只有超级电容能放电
                               Eb(i)=E_bmin;
                               Qb(i)=0;
                               if tc(i)<=1%超级电容容量达到上限
                                  Qc(i)=(Ec(i-1)-E_cmin)*c_disc;
                                  Q_bre(i)=(P_exc(i)-P_dis_maxc)+(E_cmin-(Ec(i-1)-P_disc(i)/c_disc))*c_disc;
                                  Ec(i)=E_cmin;
                               else
                                  
                                  Q_bre(i)=(P_exc(i)-P_dis_maxc);
                               end
                            end
                         end
                         if Eb(i-1)<=E_bmin&&Ec(i-1)<=E_cmin%%混合储能系统无法放电
                            Qb(i)=0;
                            Eb(i)=E_bmin;
                            Qc(i)=0;
                            Ec(i)=E_cmin;
                  ...
                  if abs(P_exc(i))<=P_cha_max  %case1:净负荷小于蓄电池最大功率且soc允许，由蓄电池补充缺额
                   ...
                   elseif abs(P_exc(i))>P_cha_max && abs(P_exc(i))<=(P_cha_max+P_cha_maxc) %case2：净负荷大于蓄电池最大功率，小于蓄电池和主网售电最大功率了之和，蓄电池以最大功率放电，剩余不足由主网补充
                        ...
                        elseif tb(i)<=tc(i)&&tb(i)<1%蓄电池先充电至最大容量
                            ...
                            if abs(P_exc(i))>=P_cha_maxc
                                Ec(i)=Ec(i-1)+((abs(P_exc(i))-P_cha(i))*tb(i)+P_cha_maxc*(1-tb(i)))*0.98;%不考虑容量限制，放电一个小时后的容量
                               if (E_cmax-(Ec(i-1)+P_chac(i)*tb(i)*0.98))/(P_cha_maxc*0.98)>(1-tb(i))%超级电容容量未达下限
                                   Q_was(i)=(abs(P_exc(i))-P_cha_maxc)*(1-tb(i));
                                   Qc(i)=-(P_chac(i)*tb(i)+P_cha_maxc*(1-tb(i)));
                               ...
                               Ec(i)=Ec(i-1)+((abs(P_exc(i))-P_cha(i))*tb(i)+abs(P_exc(i))*(1-tb(i)))*0.98;
                                if (E_cmax-(Ec(i-1)+P_chac(i)*tb(i)*0.98))/(abs(P_exc(i))*0.98)>(1-tb(i))%超级电容容量未达下限
                                   Q_was(i)=0;
                                   Qc(i)=-(P_chac(i)*tb(i)+abs(P_exc(i))*(1-tb(i)));
...
x=1:8640;
figure;
 plot(x,P_exc-(Qb+Qc+Q_bre-Q_was))
